...
class Hylax_Storage_sql extends Hylax_Storage {
    ...
    function newFaxId()
    {
        ...
        if (is_a($id, 'PEAR_Error')) {
            Horde::log('Could not generate new fax id. %s' . $id->getMessage(), 'ERR');
        } else {
            Horde::log('Generated new fax id: ' . $id, 'DEBUG');
        }
        return $id;
    }
    ...
    function _createFax(&$info)
    {
        ...
        $values = array($info['fax_id'],
                       ...
                       $info['fax_created'],
                       $info['fax_folder']);
        Horde::log('SQL Query by Hylax_Storage_sql::_createFax(): ' . $sql, 'DEBUG');
    ...
    function saveGateway(&$info)
    {
        ...
        Horde::log('SQL Query by Hylax_Storage_sql::saveGateway(): ' . $sql, 'DEBUG');
        $result = $this->_db->query($sql, $values);
        if (is_a($result, 'PEAR_Error')) {
            Horde::log($result, 'ERR');
            return $result;
    ...
    function deleteGateway($gateway_id)
    {
        ...
        $values = array((int)$gateway_id);
        Horde::log('SQL Query by Hylax_Storage_sql::deleteGateway(): ' . $sql, 'DEBUG');
        $result = $this->_db->query($sql, $values);
        if (is_a($result, 'PEAR_Error')) {
            Horde::log($result, 'ERR');
        }
    ...
    function _getMessages()
    {
        ...
        Horde::log('SQL Query by Hylax_Storage_sql::_getMessages(): ' . $sql, 'DEBUG');
